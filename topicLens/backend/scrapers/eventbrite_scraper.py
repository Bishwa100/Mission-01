...
def scrape_eventbrite(query: str, max_results: int = 10) -> list:
    ...
    # Fallback: search via DuckDuckGo if no results
    if not results:
        from .universal_search_scraper import search_duckduckgo
        search_results = search_duckduckgo(f"{query} workshop webinar eventbrite OR meetup", max_results)
        for r in search_results:
            url = r.get("url", "")
            if "eventbrite" in url or "meetup" in url:
                r["source"] = "eventbrite" if "eventbrite" in url else "meetup"
                r["type"] = "event"
                results.append(r)
